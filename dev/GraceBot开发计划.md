...
| 1 | 最小对话链路 | 待开发 | 会话管理 + 任务队列 + 多轮纯文本回复 |
| 2 | 人格与技能 | 待开发 | PromptBuilder + SOUL.md + Skills 注入 |
| 3 | ReAct 与基础工具 | 待开发 | 工具注册表 + ReAct 循环 + exec + file_read |
| 4 | 文件与网络工具 | 待开发 | file_write / file_edit / web_search / web_fetch |
| 5 | 记忆系统 | 待开发 | 向量记忆 + memory_read / memory_write + 自动检索 |
| 6 | 用户画像与技能迭代 | 待开发 | USER.md 自动更新、Skill 自我迭代 |
| 7 | 多模型与稳定性 | 待开发 | ModelRouter 容错、Compaction、HookBus/插件骨架 |
...
- [ ] 飞书问「你叫什么名字？」→ 回答与 SOUL.md 中设定一致。
- [ ] 修改 SOUL.md 内容并重启，再问同一问题 → 回复风格/内容随之变化。
- [ ] 飞书问「你有什么能力？」或「你会做什么？」→ 能说出 Skills 中描述的能力。
- [ ] 新增或修改一个 skill 文件，重启后 Bot 行为或自我描述与之相符。
...
- [ ] 飞书发送「执行 ls」或「列出当前目录文件」→ Bot 调用 exec 并返回真实命令输出。
- [ ] 飞书发送「读取 workspace 下的 xxx 文件」→ Bot 调用 file_read 并返回文件内容（或合理错误提示）。
- [ ] 工具执行失败（如命令不存在、文件不存在）时，Bot 在飞书里给出明确错误说明而非崩溃。
- [ ] 多轮工具调用（如先 ls 再读某个文件）能连续执行并最终回复正确。
...
- [ ] 飞书「在 workspace 下创建 test.txt，内容为 Hello」→ 文件真实存在且内容正确。
- [ ] 飞书「把 test.txt 里的 Hello 改成 Hi」→ 通过 file_edit 修改成功并在回复中体现。
- [ ] 飞书「搜索 2026 年 xxx 新闻」→ 调用 web_search 并基于结果总结回复。
- [ ] 飞书「抓取 https://xxx 的内容并总结」→ 调用 web_fetch 并返回解读；超长或超时时有合理截断/提示。
...
- [ ] 飞书「记住：我是 TypeScript 开发者」→ Bot 确认已记住；检查 `entries.json` 有对应内容。
- [ ] 新开会话（或 30 分钟后新会话），问「你还记得我是做什么的吗？」→ Bot 能正确回忆。
- [ ] 飞书问一个与已存记忆相关的问题（未显式说「记住」），Bot 回复中能体现相关记忆（自动检索生效）。
- [ ] 记忆内容过长或条数过多时，检索与注入有合理截断，不导致 prompt 超长报错。
...
- [ ] 与 Bot 进行多轮对话后，检查 `data/users/{id}/USER.md`，存在自动追加的用户相关描述。
- [ ] 在后续对话中问「你知道我的偏好/职业吗？」→ 回复能体现 USER.md 中的画像。
- [ ] （若实现技能迭代）通过对话或脚本触发技能更新后，Bot 在新对话中体现出新技能描述。
...
- [ ] 故意使用错误或限流的 API Key → Bot 能切换或等待后重试，并在飞书给出可理解的提示而非崩溃。
- [ ] 进行极长多轮对话或注入超长上下文 → 触发 Compaction 后仍能正常回复，不出现 context overflow 错误。
- [ ] 关闭主模型或模拟网络故障 → 若有备用模型则自动切换并回复。
- [ ] 阅读 README，能按文档完成配置、启动并在飞书中完成一次完整对话。
